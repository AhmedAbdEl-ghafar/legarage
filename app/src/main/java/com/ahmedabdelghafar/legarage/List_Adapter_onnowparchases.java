...
public class List_Adapter_onnowparchases extends BaseAdapter {
    onnowparchases onnowparchases_view;


    List_Adapter_onnowparchases(onnowparchases onnowparchases_view) { this.onnowparchases_view = onnowparchases_view;}

    @Override
    public int getCount() {
        return onnowparchases_view.countries.size();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        List_Adapter_onnowparchases.ViewHolderItem holder = new List_Adapter_onnowparchases.ViewHolderItem();
        if (convertView == null) {
            LayoutInflater inflater = (LayoutInflater) onnowparchases_view.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            convertView = inflater.inflate(R.layout.cell_onnowparchases, null);
            ...
            holder.imagess_flogss = (ImageView) convertView.findViewById(R.id.imagess_flog_onnowparchases);
           ...
           */ //holder.dr_receiving = (TextView) convertView.findViewById(R.id.dr_receiving);
            ...
            holder = (List_Adapter_onnowparchases.ViewHolderItem) convertView.getTag();
        }


        holder.id_order.setText(this.onnowparchases_view.countries.get(position).id_order);
        holder.ddate.setText(this.onnowparchases_view.countries.get(position).ddate);
        holder.discreption.setText(this.onnowparchases_view.countries.get(position).discreption);
        holder.url.setText(this.onnowparchases_view.countries.get(position).url);
...
//If the edit text contains previous text with potential links
        ...
        holder.price.setText(this.onnowparchases_view.countries.get(position).price);
        Glide.with(onnowparchases_view.getApplicationContext()).load(this.onnowparchases_view.countries.get(position).imagessflog).into(holder.imagess_flogss);

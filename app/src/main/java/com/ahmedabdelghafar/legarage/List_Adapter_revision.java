...
public class List_Adapter_revision extends BaseAdapter {
    revision revisionss;


    List_Adapter_revision(revision revisionss) { this.revisionss = revisionss;}

    @Override
    public int getCount() {
        return revisionss.countries.size();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        List_Adapter_revision.ViewHolderItem holder = new List_Adapter_revision.ViewHolderItem();
        if (convertView == null) {
            LayoutInflater inflater = (LayoutInflater) revisionss.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
            convertView = inflater.inflate(R.layout.cell_revision, null);
            ...
            holder = (List_Adapter_revision.ViewHolderItem) convertView.getTag();
        }


        holder.id_order.setText(this.revisionss.countries.get(position).id_order);
        holder.ddate.setText(this.revisionss.countries.get(position).ddate);
        holder.discreption.setText(this.revisionss.countries.get(position).discreption);
        holder.detailed_description.setText(this.revisionss.countries.get(position).detailed_description);
        holder.main_category_name.setText(this.revisionss.countries.get(position).main_category_name);
        holder.sub_category_name.setText(this.revisionss.countries.get(position).sub_category_name);
        holder.weight_st.setText(this.revisionss.countries.get(position).weight_st);
        Glide.with(revisionss.getApplicationContext()).load(this.revisionss.countries.get(position).imagessflog).into(holder.imagess_flogss);

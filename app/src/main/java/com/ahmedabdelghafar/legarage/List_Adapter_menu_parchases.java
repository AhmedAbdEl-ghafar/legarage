...
/**
 * Created by orcl on 14/10/2017.
 */

public class List_Adapter_menu_parchases extends BaseAdapter {
    menu_parchases menuss_parchases;


    List_Adapter_menu_parchases(menu_parchases menuss_parchases) { this.menuss_parchases = menuss_parchases;}

    @Override
    public int getCount() {
        return menuss_parchases.countries.size();
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        List_Adapter_menu_parchases.ViewHolderItem holder = new List_Adapter_menu_parchases.ViewHolderItem();
        if (convertView == null) {
            LayoutInflater inflater = (LayoutInflater) menuss_parchases.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
...
*/
            ...
            holder = (List_Adapter_menu_parchases.ViewHolderItem) convertView.getTag();
        }


        holder.menu_id.setText(this.menuss_parchases.countries.get(position).menu_id);
        holder.menu_name.setText(this.menuss_parchases.countries.get(position).menu_name);
        holder.menu_sum.setText(this.menuss_parchases.countries.get(position).menu_sum);

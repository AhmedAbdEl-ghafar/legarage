...
public class readyshipmentparchases extends Activity implements Download_data.download_complete {
    public ListView listView;
    public ArrayList<Countries> countries = new ArrayList<Countries>();
    public List_Adapter_readyshipmentparchases adapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_readyshipmentparchases);

        pd = new ProgressDialog(readyshipmentparchases.this);
        ...
        textviewaddress.setText("جاهزة للشحن");

        listView = (ListView) findViewById(R.id.ListView_readyshipmentparchases);
        adapter = new List_Adapter_readyshipmentparchases(this);
        listView.setAdapter(adapter);

        SharedPreferences sharedPreferences = getSharedPreferences("Data", Context.MODE_PRIVATE);

        Download_data download_data = new Download_data((Download_data.download_complete) this);
        download_data.download_data_from_link("http://197.51.253.242:3000/api/showdataonlocationparchases/"+getIntent().getStringExtra("menu_id") +","+ sharedPreferences.getString("seller_id", "0"));
...
*/
        ...
        swipeContainer = (SwipeRefreshLayout) findViewById(R.id.swiperefresh_readyshipmentparchases);
        swipeContainer.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            @Override
            public void onRefresh() {
                SharedPreferences sharedPreferences = getSharedPreferences("Data", Context.MODE_PRIVATE);
                Download_data download_data = new Download_data((Download_data.download_complete) readyshipmentparchases.this);
                download_data.download_data_from_link("http://197.51.253.242:3000/api/showdataonlocationparchases/"+getIntent().getStringExtra("menu_id") +","+ sharedPreferences.getString("seller_id", "0"));
    ...
    public void onBackPressed() {
        Intent i = new Intent(readyshipmentparchases.this, menu_parchases.class);
        //i.putExtra("dr_no",getIntent().getStringExtra("dr_no"));
        startActivity(i);
    }
